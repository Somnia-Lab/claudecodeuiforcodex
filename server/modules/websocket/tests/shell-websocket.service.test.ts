...
test('Windows Codex shells invoke the configured executable path when resuming', () => {
  const pty = createFakePty();
  const socket = createFakeSocket();
  let spawnedShell = '';
  let spawnedArgs: string[] = [];
  const dependencies = {
    resolveProviderSessionId: () => 'provider-session-123',
    platform: 'win32' as const,
    resolveWindowsCodexCommand: () => "'C:\\Program Files\\OpenAI\\Codex\\bin\\codex.exe'",
    spawnPty: (shell: string, args: string[]) => {
      spawnedShell = shell;
      spawnedArgs = args;
      return pty as never;
    },
  ...
  socket.emit(
    ...
    JSON.stringify({
      type: 'init',
      projectPath: process.cwd(),
      sessionId: `windows-codex-${Date.now()}`,
      hasSession: true,
      provider: 'codex',
    }),
  );
  ...
  assert.deepEqual(spawnedArgs, [
    '-Command',
    "& 'C:\\Program Files\\OpenAI\\Codex\\bin\\codex.exe' resume \"provider-session-123\"; if ($LASTEXITCODE -ne 0) { & 'C:\\Program Files\\OpenAI\\Codex\\bin\\codex.exe' }",
  ]);
