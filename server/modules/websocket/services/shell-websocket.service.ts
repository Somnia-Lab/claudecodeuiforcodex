...
type ShellWebSocketDependencies = {
  ...
  resolveWindowsCodexCommand?: () => string;
};
...
function buildShellCommand(
  ...
  if (provider === 'codex') {
    if (resumeSessionId) {
      if (platform === 'win32') {
        const codexCommand = dependencies.resolveWindowsCodexCommand?.() ?? getWindowsCodexCommand();
        return `& ${codexCommand} resume "${resumeSessionId}"; if ($LASTEXITCODE -ne 0) { & ${codexCommand} }`;
      }
      return `codex resume "${resumeSessionId}" || codex`;
    }
    const codexCommand = dependencies.resolveWindowsCodexCommand?.() ?? getWindowsCodexCommand();
    return platform === 'win32' ? `& ${codexCommand}` : 'codex';
  }
